...
| `<slug>-model.json` | `buildapp.canonical-building-model` 1.4.0 | the candidate's program replayed through the real DSL and serialised; the audit checks it is byte for byte the replayed model |
...
| `source-view-residuals.json` | `{ candidateHash, residuals[] }` | the model projected into the registered renders: per opening on a facade a render shows, `OPENING_SILL` and `OPENING_HEAD` against the elevation extent; per main-roof sample column, `ROOF_EDGE` against the nearest strong edge. Each residual has `modelM`, `observedM`, `residualM`, `toleranceM`, `withinTolerance` and a `why`. 28 on the reference run, 9 outside tolerance |
