...
- `sceneTree()` — Building → Levels → Walls (→ Openings → Window/Door), Rooms,
  Slabs, Roofs, Balconies (→ hosted railings), Railings, Chimneys, Stairs;
  then Materials, Constraints, Evidence sources
...
- **Inspector**: kind, id, level, host wall, opening, mesh count; Hide/Show,
  ...
  on Enter/blur); evidence status tags; raw object JSON; the last command
  error when one was rejected.
...
Six Playwright checks against `vite preview` of the production build, in
...
canonical JSON and Load restores it (a broken file is refused). Screenshots
are written to `stage-reports/artifacts/`.
