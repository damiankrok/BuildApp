...
describe('frozen fixtures', () => {
  ...
  it('the 1.1.0, 1.2.0, 1.3.0 and 1.4.0 fixtures are kept as migration baselines and are untouched', () => {
    ...
    const raw14 = JSON.parse(readFileSync(FIXTURE_1_4_0, 'utf8')) as { schemaVersion: string; linearSolids?: unknown; terraces?: unknown }
    expect(raw14.schemaVersion).toBe('1.4.0')
    expect(raw14.linearSolids).toEqual([])
    expect(raw14.terraces).toBeUndefined()
  })
})
