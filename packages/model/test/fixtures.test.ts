 ...
 * (1.2.0, roof-opening collections) and STAGE BUILDAPP-01A (1.3.0, the
 * surfaceRegions collection). Each frozen file is the fixed baseline the next
...
describe('frozen fixtures', () => {
  it('the 1.3.0 fixture is the current schema version and loads without migration or issues, byte-stable', () => {
    const text = readFileSync(FIXTURE_1_3_0, 'utf8')
    const raw = JSON.parse(text) as { schemaVersion: string; wallJunctions: unknown[]; wallRings: unknown[]; roofOpenings: unknown[]; rooflights: unknown[]; surfaceRegions: unknown[] }
  ...
  it('the 1.1.0 and 1.2.0 fixtures are kept as migration baselines and are untouched', () => {
