...
/** The same house as STAGE BUILDAPP-01A persisted it (schema 1.3.0, surface regions, no linear solids). */
...
describe('schema evolution 1.0.0 -> 1.1.0 -> 1.2.0 -> 1.3.0 -> 1.4.0', () => {
  it('the current schema version is 1.4.0 and new models carry empty topology, roof-opening, surface-region and linear-solid collections', () => {
    expect(MODEL_SCHEMA_VERSION).toBe('1.4.0')
    const m = createEmptyModel('x', 'x')
    expect(m.schemaVersion).toBe('1.4.0')
    ...
    expect(m.linearSolids).toEqual([])
    expect(validateModel(m).issues).toEqual([])
  })

  it('a BUILDAPP-00 file (1.0.0) loads through four explicit, reported migration steps and keeps every object', () => {
    ...
    expect(r.model.schemaVersion).toBe('1.4.0')
    ...
    // re-saving writes a 1.4.0 file that loads without any migration
    const saved = serializeModel(r.model)
    expect(saved).toContain('"schemaVersion": "1.4.0"')
    const again = loadModel(saved)
    expect(again.ok && again.issues).toEqual([])
  })

  it('a BUILDAPP-00A file (1.1.0) loads through three explicit, reported migration steps: openings keep level heads and single leaves', () => {
    ...
    expect(r.model.surfaceRegions).toEqual([])
    expect(r.model.linearSolids).toEqual([])
    expect(r.model.meta.notes).toEqual([MIGRATION_NOTE_1_1_0, MIGRATION_NOTE_1_2_0, MIGRATION_NOTE_1_3_0])
  ...
  it('a BUILDAPP-01 file (1.2.0) loads through two explicit, reported migration steps: slabs solid, roof cuts vertical, doors one leaf, stairs placeholders', () => {
    ...
    expect(r.model.surfaceRegions).toEqual([])
    expect(r.model.linearSolids).toEqual([])
    expect(r.model.meta.notes).toEqual([MIGRATION_NOTE_1_2_0, MIGRATION_NOTE_1_3_0])
  ...
  it('a BUILDAPP-01A file (1.3.0) loads through one explicit, reported migration step: no facade member becomes a solid', () => {
    ...
    const r = loadModel(text(FIXTURE_1_3_0))
    expect(r.ok).toBe(true)
    if (!r.ok) return
    ...
    expect(r.model.meta.notes).toEqual([MIGRATION_NOTE_1_3_0])
    const saved = serializeModel(r.model)
    expect(saved).toContain('"schemaVersion": "1.4.0"')
    expect(loadModel(saved).issues).toEqual([])
  })
  ...
  it('a file that states an older version but already carries newer collections is refused', () => {
    ...
    const latest = JSON.parse(text(FIXTURE_1_3_0)) as Record<string, unknown>
    latest.linearSolids = []
    const r4 = validateModel(latest)
    expect(r4.ok).toBe(false)
    expect(r4.issues[0]).toMatchObject({ code: 'SCHEMA', path: 'schemaVersion' })
    expect(r4.issues[0].message).toContain('linearSolids')
  })

  it('unknown schema versions are refused by name, never reinterpreted', () => {
    for (const v of ['0.9.0', '1.5.0', '2.0.0', 1.1]) {
      ...
      expect(r.issues[0].message).toContain('1.0.0, 1.1.0, 1.2.0, 1.3.0, 1.4.0')
    }
  })
