...
describe('schema evolution 1.0.0 -> 1.1.0 -> 1.2.0', () => {
  it('the current schema version is 1.2.0 and new models carry empty topology and roof-opening collections', () => {
    expect(MODEL_SCHEMA_VERSION).toBe('1.2.0')
    const m = createEmptyModel('x', 'x')
    expect(m.schemaVersion).toBe('1.2.0')
  ...
  it('a BUILDAPP-00 file (1.0.0) loads through two explicit, reported migration steps and keeps every object', () => {
    ...
    expect(r.model.schemaVersion).toBe('1.2.0')
    ...
    // re-saving writes a 1.2.0 file that loads without any migration
    const saved = serializeModel(r.model)
    expect(saved).toContain('"schemaVersion": "1.2.0"')
    const again = loadModel(saved)
    expect(again.ok && again.issues).toEqual([])
  })

  it('a BUILDAPP-00A file (1.1.0) loads through one explicit, reported migration step: openings keep level heads and single leaves', () => {
    ...
    expect(r.model.roofOpenings).toEqual([])
    expect(r.model.rooflights).toEqual([])
    expect(r.model.meta.notes).toEqual([MIGRATION_NOTE_1_1_0])
  ...
  it('unknown schema versions are refused by name, never reinterpreted', () => {
    for (const v of ['0.9.0', '1.3.0', '2.0.0', 1.1]) {
      ...
      expect(r.issues[0].message).toContain('1.0.0, 1.1.0, 1.2.0')
    }
  })
