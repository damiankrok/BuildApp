 ...
 *
 * The analyzer's packages are content-addressed: a SourcePackage and a
 * SourceObservationGraph each carry a hash of their own canonical content, and
 * observation ids are derived from what they describe. Those hashes must be
 * identical in Node, in a browser and in a test, and must be computable
 * synchronously (WebCrypto's digest is async, `node:crypto` is not portable),
 * so the one implementation lives here.
 *
 * FIPS 180-4. Operates on bytes; `sha256Hex` of a string hashes its UTF-8.
 */
...
export function sha256Bytes(input: Uint8Array): string {
  ...
  // bit lengths beyond 2^53 cannot occur for anything we hash; write as two 32-bit halves
...
export const sha256Hex = (text: string): string => sha256Bytes(UTF8.encode(text))

/** A short content address: `sha256:` and the first `n` hex characters (16 by default: 64 bits, ample for one project's assets). */
export const shortHash = (hex: string, n = 16): string => `sha256:${hex.slice(0, n)}`
